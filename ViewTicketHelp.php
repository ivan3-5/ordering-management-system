...
<head>
    ...
    <link rel="stylesheet" href="ViewTicketHelp.css">
</head>
<body>
    <a href="homepage.php">
...
<div class="new-bg-container">
    <?php
        ...
        $randomRole = $roles[random_int(0, count($roles) - 1)];
        // Generate a random alphanumeric ticket number
        $ticketID = generateTicketID(8); // You can change the length if needed
        $todayDate = date("F j, Y");

    // Logic with a break statement
    switch ($randomRole) {
        case "Admin":
            $openedBy = "Opened by: Admin";
            break;
        case "Employee":
            $openedBy = "Opened by: Employee";
            break;
        default:
            $openedBy = "Opened by: Unknown"; // Fallback in case of unexpected role
            break;
    }
    ?>
...
<p class="ticket-text">
    You have your ticket number: <?php echo $ticketID; ?>  
    Date: <?php echo $todayDate; ?>  <br>
    <?php echo $openedBy; ?>
</p>
